...
```bash
[root@blog ~]# tac /etc/issue

Kernel \r on an \m
\S
[root@blog ~]# cat /etc/issue
\S
Kernel \r on an \m
```
...
```bash
[root@blog ~]# more /etc/sysconfig/network-scripts/ifdown-routes 
#! /bin/bash
#
...
```bash
[root@blog ~]# find [PATH] [option] [action]
选项与参数：
...
[root@blog ~]# find /root/Console_setup/ -mtime 0
...
[root@blog ~]# find /PATH -user USERNAME
